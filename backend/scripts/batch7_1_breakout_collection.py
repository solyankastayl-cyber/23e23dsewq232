#!/usr/bin/env python3
"""
Batch 7.1: BUY-Only Collection (Breakout Long)
===============================================

Phase A.9.2: Test Breakout BUY engine (regime-agnostic).
...
Goal: Prove viability in ANY market regime (not uptrend-dependent).
"""
...
async def create_shadow_trade(db, signal: Dict[str, Any], horizon_hours: int = 4, experiment_id: str = "batch7_1_breakout_buy") -> str:
...
async def generate_breakout_signals(eligible: List[Dict[str, Any]], market_data) -> List[Dict[str, Any]]:
    """Generate BUY signals using breakout logic."""
    signals = []
    ...
    for asset in eligible:
        try:
            ...
            if not candles or len(candles) < 21:  # Need lookback_period + 1
                continue
            
            # Create generator
            generator = BreakoutLongGenerator(symbol=symbol, timeframe=timeframe)
...
async def batch71_collection_loop(
    target_trades: int = 30,
    horizon_hours: int = 4,
    cycle_interval_minutes: int = 15,
    experiment_id: str = "batch7_1_breakout_buy"
):
    """
    Batch 7.1 collection loop: BUY-only with breakout generator.
    """
    print("="*70)
    print("🔬 BATCH 7.1: BUY-ONLY COLLECTION (Breakout Long)")
    print("="*70)
    ...
    print(f"Generator: breakout_long_v1")
    print(f"Mode: BUY ONLY (regime-agnostic)")
    print("="*70)
    print()
    ...
    while True:
        ...
        try:
            ...
            # Step 3: Generate BUY signals (breakout)
            print(f"[3/4] Generating BUY signals (breakout)...")
            signals = await generate_breakout_signals(eligible, market_data)
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(description="Batch 7.1: BUY-only collection (Breakout)")
    ...
    parser.add_argument("--experiment", type=str, default="batch7_1_breakout_buy", help="Experiment ID")
    
    args = parser.parse_args()
    
    asyncio.run(batch71_collection_loop(
