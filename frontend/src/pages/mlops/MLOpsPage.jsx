...
export default function MLOpsPage() {
  ...
  const handleAction = async (action, payload) => {
    // ─────────────────────────────────────────────────────────────
    // V-3 + V-4 RE-MOUNT (2026-04-29): operator safety guard.
    // The five actions below mutate production ML state. They are
    // reachable only from /admin/tech-analysis → MLOps now (the
    // /mlops user route was removed in MOVE V-4). To match the
    // safety pattern already applied to Rollback originally, every
    // action here now requires explicit confirm. This is the ONLY
    // logic change introduced by the re-mount.
    // ─────────────────────────────────────────────────────────────
    const ACTION_DESCRIPTIONS = {
      retrain:  'Retrain the candidate model on the latest data.\nThis spawns a new training run and may consume compute budget.',
      promote:  'Promote the candidate model to ACTIVE.\nThis replaces the model currently serving production predictions.',
      rollback: 'Roll back the ACTIVE model to the previous version.\nIncoming predictions will immediately use the older model.',
      retire:   'Retire the selected model.\nA retired model cannot be promoted again without explicit re-instatement.',
      evaluate: 'Run a shadow-mode evaluation pass.\nNo production state is changed by this action.',
    };
  ...
  return (
    <div className="min-h-screen bg-gray-50 text-gray-900 p-6">
      {/* V-3 + V-4 RE-MOUNT operator banner — 2026-04-29.
          Indicates that this surface mutates production ML state.
          Pure safety UX, no logic. */}
      <div className="mb-4 p-3 border border-red-300 bg-red-50 rounded-lg flex items-start gap-2"
           data-testid="mlops-operator-banner">
        <AlertTriangle className="w-4 h-4 text-red-600 flex-shrink-0 mt-0.5" />
        <div className="text-sm">
          <strong className="text-red-700">Operator Console.</strong>
          <span className="text-red-700/90 ml-1">
            Changes here affect production ML state. Each action requires explicit confirmation.
          </span>
        </div>
      </div>
