...
// MOVE V-1-USER-CLOSE (2026-04-29): Calibration components are no
// longer rendered from user space. The four canonical components
// (CalibrationStatusCard / CalibrationBuildPanel /
// CalibrationRunHistory / CalibrationAttackTests) live untouched in
// /components/calibration/ and are now consumed exclusively by the
// admin tree at /admin/tech-analysis → "Calibration" sub-tab.
// Imports removed here on purpose. See /app/memory/MOVE_V1_USER_CLOSE.md.
//
    ...
    <div className="flex items-center gap-2 mb-4">
      ...
      <TooltipProvider>
        <Tooltip>
          ...
          <TooltipContent className="max-w-xs bg-white text-white border-gray-200">
...
function TabNav({ active, onChange }) {
  const tabs = [
    ...
    // MOVE V-1-USER-CLOSE (2026-04-29): "Calibration" tab removed
    // from user TabNav. Calibration control now lives only at
    // /admin/tech-analysis → Calibration sub-tab.
    // Removed entry was:
    ...
    <div className="space-y-6">
      ...
      <TooltipProvider>
        <Tooltip>
          ...
          <TooltipContent className="max-w-sm bg-white text-white border-gray-200">
...
// MOVE V-1-USER-CLOSE (2026-04-29): The CalibrationTab function was
// removed from this file. It used to render the four canonical
// Calibration cards (Status / Build / History / Attack Tests) from
// /components/calibration/ as a user-side tab. Those cards are now
// owned exclusively by the admin tree:
//   /admin/tech-analysis → "Calibration" sub-tab (V-1 RE-MOUNT).
//
// Reasons:
// 1. Calibration exposes operator-grade mutations (build / simulate
//    / apply / disable) that must not be reachable from a regular
//    user surface.
// 2. Keeping a parallel user copy duplicates the operator surface
//    and would let a non-admin trigger a state change.
//
// What was deleted is preserved verbatim in the canonical files —
// nothing of the actual UI was lost. See:
//   /components/calibration/CalibrationStatusCard.jsx
//   /components/calibration/CalibrationBuildPanel.jsx
//   /components/calibration/CalibrationRunHistory.jsx
//   /components/calibration/CalibrationAttackTests.jsx
//
// Audit trail: /app/memory/MOVE_V1_USER_CLOSE.md.
  ...
  return (
    <div className="min-h-screen bg-gray-50">
      ...
      <main className="max-w-5xl mx-auto px-4 py-6">
        ...
        {/* MOVE V-1-USER-CLOSE (2026-04-29): activeTab === 'calibration'
            render line removed. Calibration UI is now admin-only at
            /admin/tech-analysis → Calibration. If a stale URL still
            sets activeTab to 'calibration', no panel renders (safe).
            Old line: {activeTab === 'calibration' && <CalibrationTab />} */}
