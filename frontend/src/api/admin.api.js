...
export async function getSystemOverview() {
  // GRACEFUL 2026-04-30: /api/admin/system/overview is not yet ported to the
  // FastAPI backend (legacy TS handler). Returning a typed empty payload keeps
  // SystemOverviewPage from emitting a red console error on every mount.
  // Remove this guard once the backend route exists.
  try {
    return await adminFetch('/api/admin/system/overview');
  } catch (err) {
    if (err && (err.status === 404 || /404/.test(String(err.message)))) {
      return { ok: false, status: 'unavailable', overview: null };
    }
    throw err;
  }
}
