...
export default function ExchangePressureCard({ network = 'ethereum', window = '24h' }) {
  ...
  const fetchData = useCallback(async () => {
    ...
    try {
      ...
      // Gracefully handle missing endpoint (exchange-pressure may not be implemented yet)
  ...
  // BUG FIX 2026-04-30: data may be null when /api/market/exchange-pressure is
  // 404 on backend (legacy endpoint, not yet ported). Render a calm empty
  // state instead of a half-empty Exchange Pressure card.
  if (!data) {
    return (
      <div className="bg-white rounded-xl p-6 border border-gray-200">
        ...
        <p className="text-sm text-gray-500">
          Data unavailable. The exchange-pressure feed is not active on the
          current backend.
        </p>
      </div>
    );
